...
int main (void) {
    ...
    PCA9555_0_write(REG_CONFIGURATION_0, 0x00); // IO as output to LCD
    ...
    while (1) {
        ...
        while (digit_ctr < 2) {
            while ((key = keypad_to_ascii().all_pressed) == '-'); 
            pwd[digit_ctr++] = key; 
            _delay_ms(250); 
            lcd_data('*'); 
            while (keypad_to_ascii().all_pressed == key);  // avoid double count
        }
        ...
        if (pwd[0] == '6' && pwd[1] == '0') {
            lcd_string("GRANTED ^_^"); 
            PORTB = 0xFF; 
            _delay_ms(3000); 
            PORTB = 0x00; 
        }
        else {
            lcd_string("DENIED X_X");
            for (int i = 0; i < 6; i++) {
                PORTB = 0xFF; 
                _delay_ms(500); 
                PORTB = 0x00; 
