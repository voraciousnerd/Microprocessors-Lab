...
* "pressed_keys" -> all keys currently pressed
* "new_keys" -> the keys that were newly pressed since the last scan
* Thus, we can handle continuous actions (holding a key) and track new keys,
* which is useful for this and future exercises.
*
*/
typedef struct {
...
int main (void) {
    ...
    char key; 
    uint8_t output = 0; 
    PCA9555_0_write(REG_OUTPUT_0, output); // initialize output to NULL
    
    while(1) {
        key = keypad_to_ascii().all_pressed; 
        if (key == '-') {
            PORTB = 0x00; 
            continue; 
        }
        ...
        switch(key) {
            case '4': output = 0x02; break;
            case '2': output = 0x04; break;
            case '3': output = 0x08; break;
            case 'B': output = 0x10; break;
            default: output = 0; break; 
        }
        PORTB = output; 
        
        while (keypad_to_ascii().all_pressed == key); 
    }
    return 0; 
}